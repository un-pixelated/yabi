...
int validate_number_of_arguments(int num_args, const char* binary_name) {
    if (num_args < 2) {
        std::fprintf(stderr, ERR_NUMBER_OF_ARGS, binary_name);
        return 1;
    }
    return 0;
}
...
int validate_file_extension(const char* filename) {
    if (extension_checker(filename) == 0) {
        std::fprintf(stderr, ERR_WRONG_FILE_EXTENSION, filename);
        return 1;
    }
    return 0;
}
...
int validate_file_not_empty(FILE** file_ptr, int file_character) {
    if (file_character == EOF) {
        std::fprintf(stderr, ERR_EMPTY_FILE);
        std::fclose(*file_ptr);
        return 1;
    } else {
        std::rewind(*file_ptr);
